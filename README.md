...
A collection of hands-on Linux administration exercises: user & permission management, storage (LVM/NFS), automation (Ansible), networking (bonding/ VLANs), name services (DNS, FreeIPA/LDAP), and running services in Docker with LDAP-backed authentication (Grafana).
...
| [07](07-dns-server/)              | DNS Server               | BIND, zone records, `nslookup`/`dig`, reverse lookups         | ⚠️ notes not captured |
...
Exercises were run across a small set of RHEL-family lab servers, generally in pairs so that server-side and client-side steps could be practiced together (e.g. NFS export on one host, mount on the other).
